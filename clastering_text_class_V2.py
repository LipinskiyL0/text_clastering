...
self.levels - перечень номеров уровней кластеризации. Название столбца отвечающего
                за соотвествующий уровень кластеризации образуется как 
                'level_'+str(номер уровня)
self.opt - параметры генетического алгоритма
В эту версию включаем автокодировщики. Общая идея повысить информативность признаков за счет применения автокодировщиков
Переносим векторизацию текста в класс clastering_text
...
class clastering_text:  
    ...
    def __init__(self, df, v_metrics='tfidf', opt={'n_ind':10, 'n_tur':3, 'n_iter':2}):
    ...
    def text2vec(self):   
        text=list(self.df[self.name+'_txt'])
        v_metrics=self.v_metrics
        if v_metrics=='tfidf':
    ...
    def clastering(self, level=0, claster={0:0}):
        ...
        options=self.opt
        if level==0:
            clastering=my_ga_clastering_v2( self.df_vec)
            ind, er=clastering.optimize(options)
            ...
            for k in claster:
                if flag==0:
                    ...
                    ind=ind1&ind
            df1= self.df_vec[ind.values].copy()
            clastering=my_ga_clastering_v2(df1)
            ind2, er=clastering.optimize(options)
            df1['level'+'_'+str(level)]=clastering.best_cluster_labels
            df1['index']=self.df.loc[ind, 'index'].values
            df1=df1[['index', 'level'+'_'+str(level)]]
            self.df=pd.merge(self.df, df1, how='outer',left_on='index', right_on='index')
        self.levels.append(level)    
        return
    def centr_clastering(self):
        #на текущих результатах кластеризации находим центры полученных кластеров
        #Центр кластера это усредненный вектор построенный по всем векторам кластера
        pass
...
if __name__ == '__main__':
    ...
    opt={'n_ind':5, 'n_tur':3, 'n_iter':2}
    cl=clastering_text(df[[name, name+'_txt']], v_metrics='tfidf', opt=opt)
